...
uses
  System.Win.ComObj, System.Variants, Winapi.psAPI, Winapi.Windows;
{$ENDIF}
...
begin
  ...
  begin
    ...
    Result._Physical := dwTotalPhys;        // Total physical memory in bytes
    Result._Virtual := dwAvailPhys;         // Available physical memory in bytes
    Result._PageFile := dwTotalPageFile;    // Total page file in bytes
...
begin
  ...
  if Result.SystemCpuPct < 0 then Result.SystemCpuPct := 0;
  if Result.SystemCpuPct > 1 then Result.SystemCpuPct := 1;
  if Result.ProcessCpuPct < 0 then Result.ProcessCpuPct := 0;
  if Result.ProcessCpuPct > 1 then Result.ProcessCpuPct := 1;
...
begin
  ...
  Samples.AddBytesGauge('system.memory.total', trunc(Memery._Physical));
  Samples.AddBytesGauge('system.memory.actual.free', trunc(Memery._Virtual));
  ...
  Samples.AddPercentageGauge('system.cpu.total.norm', CpuUsage.SystemCpuPct * 100);
  Samples.AddPercentageGauge('system.process.cpu.total.norm', CpuUsage.ProcessCpuPct * 100);
end;
